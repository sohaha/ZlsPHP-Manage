...
class Manage extends \Zls\Command\Command
{
    ...
    {
        $data = [
            'url' => 'https://resources.73zls.com/vue-admin-template.tar.gz',
            'md5' => '',
            'path' => Z::realPathMkdir('./manage', true, false, true),
            'ignore' => [],
            'moveRule' => [],
            'KeepOldFile' => true,
        ];
        ...
        $old = count($res['oldfile']);
        if ($old) {
            $this->warning("{$old} old file is overwritten");
        }
    ...
    private function passwd($args)
    {
        ...
        if (!$newPassword = Z::arrayGet($args, ["-p", "p"])) {
            $this->error("Password cannot be empty, please use -p {password}", "", true);
        }
        $UserDao = new UserDao();

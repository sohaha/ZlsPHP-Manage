...
<template>
  <div v-loading="ml_listsLoading || loadGroup">
    ...
    <div class="tip-area" v-if="!gid">
      温馨提示: 如不熟悉请不要随意更改权限；路由不区分大小写
    </div>
    <div class="tip-area" v-else>
      温馨提示:
      如不熟悉请不要随意更改路由权限；优先级“禁止”最高->标识码->路由通行
    </div>
    <fieldset v-if="gid">
      <legend>角色信息</legend>
      <aside v-loading="loadGroup">
        <div class="tip-area">
          当前角色下共有{{ ginfo.user_count }}名用户，生效规则共
          {{ rulesCount }} 条。
        </div>
        <div>
          <div>
        ...
        >
          ...
          <el-table-column label="规则名称" width="160">
            <template slot-scope="scope">
              ...
              <div v-else class="text-nowrap" :title="scope.row.title">
                {{ scope.row.title || " - " }}
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="类型" width="130">
            <template slot-scope="scope">
              ...
              <el-tag
                ...
                v-else
                >{{ ruleType(scope.row.type) }}</el-tag
              >
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              ...
              <div v-else class="text-nowrap" :title="scope.row.remark">
                {{ scope.row.remark || " - " }}
              </div>
            </template>
          </el-table-column>
            ...
            <template slot="header" slot-scope="scope">
              ...
              >
                <el-form-item
                  ...
                  :style="
                    ml_searchKey ? 'width:' + (gid ? 190 : 110) + 'px' : ''
                  "
                >
                  <el-input

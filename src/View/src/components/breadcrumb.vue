...
<template>
  <el-breadcrumb class="breadcrumb" separator-class="el-icon-arrow-right">
    <el-breadcrumb-item v-for="(v,k) in breadcrumb" :key="k" :class="breadcrumbClass(v)" @click.native="go(v.index)"><i class="icon-home" v-if="k===0"></i> {{v.title}}</el-breadcrumb-item>
  </el-breadcrumb>
</template>
    ...
    for (var k = 0, l = navs.length; k < l; k++) {
      ...
      if (path === current) {
        ...
        var child = getParent(
          child,
          current,
          parent, [].concat(lastNav || [], [
            { title: nav.title, index: nav.real ? nav.index : '', show: nav.show !== false, breadcrumb: nav.breadcrumb !== false }
          ])
        );
        if (child) {
          return child;

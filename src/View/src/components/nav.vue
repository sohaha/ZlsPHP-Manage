    ...
    >
      <template v-for="(v, k) in data">
        ...
        >
          <template slot="title">
            <i
              v-if="typeof v.icon !== 'undefined'"
              :class="v.icon || 'icon-flag'"
            ></i>
          ...
          >
            <i
              v-if="typeof vv.icon !== 'undefined'"
              :class="vv.icon || 'icon-flag'"
            ></i>
            {{ vv.title }}
          </el-menu-item>
        </el-submenu>
...
Spa.define({
  ...
  methods: {
     ...
     * 改变接口返回数据
     */
    changeData: function (data, isChild, parIndex) {
      if (isChild) {
        for (var i in data) {
          if (this.userNav[parIndex].child[i].show) {
            this.userNav[parIndex].child[i].show = true;
          } else {
            this.userNav[parIndex].child[i].show = false;
          }
          if (this.userNav[parIndex].child[i].real) {
            this.userNav[parIndex].child[i].real = true;
          } else {
            this.userNav[parIndex].child[i].real = false;
          }
          if (this.userNav[parIndex].child[i].breadcrumb) {
            this.userNav[parIndex].child[i].breadcrumb = true;
          } else {
            this.userNav[parIndex].child[i].breadcrumb = false;
          }
        }
      } else {
        for (var i in data) {
          if (this.userNav[i].show) {
            this.userNav[i].show = true;
          } else {
            this.userNav[i].show = false;
          }
          if (this.userNav[i].real) {
            this.userNav[i].real = true;
          } else {
            this.userNav[i].real = false;
          }
          if (this.userNav[i].breadcrumb) {
            this.userNav[i].breadcrumb = true;
          } else {
            this.userNav[i].breadcrumb = false;
          }
          var child = this.userNav[i].child;
          if (child) {
            $this.changeData(child, true, i);
          }
        }
      }
    },
